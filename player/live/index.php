...
$json = getChannels($iptv,$epgDb);


if(!isset($json) || !$json || $json=="[]" || $json==""|| count($json)==0)
    header("Location: http://{$_SERVER['HTTP_HOST']}/{$path}dashboard.php");

if($epgMode!=4 && !$epgDb && filter_var($epgUrl, FILTER_VALIDATE_URL) !== false)
	    $epgDl=getEpg($epgUrl,$result[0],$shift);

if($epgDl==""||!isset($epgDl))
    $epgDl="[]";
if(!isset($h24) || $h24==0)
    $h24=24;
    ...
    <head>
        ...
        <body>
            <script>
                ...
                var epgJson = <?php echo $epgDl; ?>; // Don't forget the extra semicolon!
