...
async function start(){
    ...
    if(!epgJson||epgJson.length===0 && shortEpg!=2)
...
async function playChannel(channelIndex, group, force=false){
    ...
    var url = channel[selectedChannel]["IP"];
        ...
        if(!url.includes("/live/")){
            ...
            var exploded = url.replace(http,"").split("/",1)[0];
            url = url.replace(exploded,exploded+"/live");
        }
    ...
    if(epgJson)
         epgBuilder(channelIndex,channel[selectedChannel]);
    if(mobile){
        closeNav();
    }
}
